...
class DynamicTrainingArguments(TrainingArguments):
    ...
    emb_batch_size: int = field(
        default=4,
        metadata={"help": "Bacth size for generated embeddings"}
    )

    emb_learning_rate: float = field(
        default=1e-5,
        metadata={"help": "Learning rate for generated embeddings"}
    )

    kd_temperature: float = field(
        default=0.4,
        metadata={"help": "Temperature scaling of Knowledge Distillation"}
    )

    lbl_calib: bool = field(
        default=False,
        metadata={"help": "label calibration by Knowledge Distillation"}
    )
...
def main():
    ...
    ###### added to training_args
    training_args.data_args = data_args
    training_args.config = config
    if data_args.prompt:
        training_args.label_word_list = torch.tensor(train_dataset.label_word_list).long().cuda()
    else:
        training_args.label_word_list = []
    ...
    training_args.few_shot_type = model_args.few_shot_type
    training_args.model_type = config.model_type
    # training_args.seed
    ...
    with FileLock('log.lock'):
        with open('log', 'a') as f:
            ...
            try:
                final_result.pop('label_word_list')   ### added for gather_result.py
                final_result.pop('data_args')
                final_result.pop('model_args')
                final_result.pop('config')
            except:
                pass
