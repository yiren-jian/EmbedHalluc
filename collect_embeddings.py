...
def main():
    ...
    elif model_args.few_shot_type == 'finetune':   ######   embedding layer is the same for either prompt or finetuning method
        if config.model_type == 'roberta':
            model_fn = RobertaForPromptFinetuning
        elif config.model_type == 'bert':
            model_fn = BertForPromptFinetuning
    ...
    model = model.cuda()
    data_len = len(train_dataset)    ### number of data points in the training set
    embeddings_by_class = {}
    for idx in range(data_len):
        data = train_dataset.__getitem__(idx)
        input_ids = torch.tensor([data.input_ids]).cuda()
        label = data.label    #### labe is here
        if data.attention_mask is not None:
            attention_mask = torch.tensor([data.attention_mask]).cuda()
        if data.mask_pos is not None:
            mask_pos = torch.tensor([data.mask_pos]).cuda()

        if 'prompt' in model_args.few_shot_type:
            if config.model_type == 'bert':
                embed_for_idx = model.bert.embeddings(
                                    input_ids=input_ids,
                                    position_ids=None,
                                    token_type_ids=None,
                                    inputs_embeds=None)
            elif config.model_type == 'roberta':
                embed_for_idx = model.roberta.embeddings(
                                    input_ids=input_ids,
                                    position_ids=None,
                                    token_type_ids=None,
                                    inputs_embeds=None)
                ...
                raise NotImplementedError
        elif 'finetune' in model_args.few_shot_type:      ######   embedding layer is the same for either prompt or finetuning method
            if config.model_type == 'bert':
                embed_for_idx = model.bert.embeddings(
                                    input_ids=input_ids,
                                    position_ids=None,
        ...
        if label in embeddings_by_class:
            embeddings_by_class[label].append(embed_for_idx.detach().data.cpu().numpy().tolist())
        else:
            embeddings_by_class[label] = []
            embeddings_by_class[label].append(embed_for_idx.detach().data.cpu().numpy().tolist())

    if not os.path.exists('cache_embeddings'):
        os.mkdir('cache_embeddings')
    ##### TODO: file name
    with open('./cache_embeddings/%s_%s_%s_%s.json'%(model_args.model_name_or_path, model_args.few_shot_type, data_args.task_name, training_args.seed), 'w') as outfile:
        json.dump(embeddings_by_class, outfile)
